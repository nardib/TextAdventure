...
Before compiling the code form source, make sure you have Java Developer Kit 17 (or above) correctly installed on your machine, otherwise your machine won't be able to compile or excecute the game. The dependency are managed with Maven, but there is no need to have it installed on you machine: in fact you can use the Maven Wrapper configured in the repository just replacing the keyword `mvn` with `./mvnw` where specified in the following instructions.
...
You can also download the zip file from https://github.com/nardib/TextAdventure: make sure to extract the content of the zip file in order to use it.
...
**NOTE**: excecute the following commands form the root folder of the project.

For compiling the code you can use Maven:
...
For creating the jar file you can use:

```bash
mvn package
```

This last command will also compile and test the code.

You can find all the file generated by the previous commands in the `target` folder.

You can (compile and) excecute the file with the command:
...
```
if you have exectued the `package` command.
